    ...
    try:
        ...
        for dt, dd in zip(dts, dds):
            try:
                ...
                article_id = id_elem[0].text.strip().replace(
                    "arXiv:", "").strip() if id_elem else ""
                if not article_id:
                    continue

                # Парсим название статьи
                title = dd.find_elements(
                ...
                subjects = re.sub(r'^Subjects:\s*', '',
                                  subjects, flags=re.IGNORECASE).strip()
